...
def _parse_card_data(raw: str) -> dict[str, Any] | None:
    ...
    Elox SDK card layout (48 chars):
      hotel_id(4) + card_id(2) + room_sdk(8) + serial(1) +
      checkin(12, yyyyMMddHHmm) + checkout(12) + byte(1) + auth(8)

    Checkout format differs from checkin for months 1–9:
      months 1–9:   yyyy + '1' + M + ddHHmm  (M is single digit, no leading zero)
      months 10–12: standard yyyyMMddHHmm
    ...
    try:
        ...
        room_sdk = raw[6:14]
        serial   = raw[14:15]
        checkin  = raw[15:27]   # yyyyMMddHHmm — standard
        co_raw   = raw[27:39]   # SDK variant format — see docstring
        ...
        # Months 1–9: bytes [4:6] will be 13–19 (extra '1' byte + single-digit month).
