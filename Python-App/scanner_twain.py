...
def _log_detection_banner(sources: list[str]) -> None:
    bar = "=" * 5
    logger.info("%s TWAIN SCANNER DETECTION %s", bar, bar)
    if not sources:
        logger.info("Available devices:\n  (none)")
    else:
        lines = "\n".join(f"  - {name}" for name in sources)
        logger.info("Available devices:\n%s", lines)
    logger.info("%s%s%s", bar, "=" * 29, bar)


def _log_scan_banner(*, device: str, simulated: bool) -> None:
    bar = "=" * 5
    logger.info("%s START SCAN %s", bar, bar)
    mode = "simulation (sample file)" if simulated else "TWAIN"
...
def _log_scan_done(*, success: bool, simulated: bool) -> None:
    ...
    if success:
        if simulated:
            logger.info("Sample image loaded (no hardware scan).")
        else:
            logger.info("Scan completed successfully")
    else:
        logger.info("Scan did not complete successfully")
    logger.info("%s%s%s", bar, "=" * 29, bar)
